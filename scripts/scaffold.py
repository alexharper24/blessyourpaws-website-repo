...
.section-cta p{margin:0;color:var(--forest-soft);font-size:.95rem}
...
/* .tri>div paints itself white, so the band's light text must not reach inside it.
   Revert to the page palette within any card sitting on a dark band. Measured on white:
   forest 11.85:1, the sage tick 3.47:1 against a 3:1 graphic floor. */
.band-forest .tri>div{color:var(--forest)}
.band-forest .tri>div h3{color:var(--forest)}
.band-forest .tri>div p,.band-forest .tri>div li{color:var(--forest)}
.band-forest .tri>div a{color:var(--sage-deep)}
.band-forest .tri>div .checklist li::before{border-color:var(--sage)}
...
.partner .btn-row+.fine{margin-top:.85rem}
...
@media (min-width:901px){
  /* `auto 1fr`, not `auto auto`: a row-spanning item spreads its extra height across the
     auto rows it spans, which re-inflates row 1 and reopens the gap this is fixing. All
     the slack goes to row 2 instead, so row 1 is exactly its own content's height. */
  .puppy-top{grid-template-rows:auto 1fr;align-items:start}
  .carousel{grid-column:1;grid-row:1}
  .puppy-about{grid-column:1;grid-row:2}
  .puppy-info{grid-column:2;grid-row:1 / span 2}
}
@media (min-width:1200px){
  /* Wide: the original arrangement, About tucked under the buy panel on the right, with
     the carousel spanning instead. The left column here is ~750px, which would give About
     lines of roughly 95 characters. */
  .carousel{grid-row:1 / span 2}
  .puppy-info{grid-column:2;grid-row:1}
  .puppy-about{grid-column:2;grid-row:2;max-width:none}
}
