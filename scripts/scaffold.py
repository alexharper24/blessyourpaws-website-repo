...
@media (max-width:900px){
  ...
  /* align-self MUST be stretch. `.hero-drift` sets align-items:center, and the copy
     shares this grid cell, so on a device whose text runs taller than min-height the
     cell grows, a centred photo stays at min-height, and the copy ends up hanging off
     the bottom of the picture it is supposed to be sitting on. Stretch keeps the photo
     as tall as whatever the copy needs. */
  .hero-photo{width:auto;justify-self:auto;margin:0;position:relative;
    aspect-ratio:auto;min-height:min(142vw,34rem);grid-area:1/1;align-self:stretch}
  ...
  /* `.wrap` makes its gutter with width:94% and auto margins, not padding, and `.hero`
     is the one section with padding:0. Everywhere else the section padding adds to that
     3%, so body copy sits about 28px in; here it left the headline 12px from the screen
     edge, which over a photograph reads as text jammed against the glass. Give the
     overlay a real gutter of its own. */
