...
open:
  ...
  - {id: g5-retarget, gate: G5, blocked_on: alex, item: "Retarget the three existing money pages, no new URLs: breed guide keeps munchkin bernedoodle, puppies takes munchkin bernedoodles for sale plus bernedoodle puppies indiana, homepage moves to the breeder terms so it stops competing with puppies. Plan in source-files/semrush/keyword-plan.md. Needs Alex's approval of the new titles"}
  - {id: g5-cluster-colors, gate: G5, blocked_on: alex, item: "Coat colors page (merle, blue merle, phantom, parti, red, tri). New from Semrush, all low difficulty, and built from the litter's own recorded colors and photographs"}
