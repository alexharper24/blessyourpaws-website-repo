...
open:
  ...
  - {id: owner-editor, gate: G5, blocked_on: alex, item: "PLANNED, not started (Alex, 2026-09-24: eventually an editor like Teapup and Sweet Puppy Paws so Hope manages the site herself). Move MUNCHKINS, ADOPTED, RESERVED, prices and parents out of scaffold.py into data/*.json owned by D1, add the admin behind Cloudflare Access on the /admin and /api routes already reserved in worker/index.js, and reuse the breeder-site-backend architecture. Once it exists the database owns those files, per the root CLAUDE.md rule"}
...
closed:
  ...
  - {id: g5-cluster-sourced, closed: 2026-09-24, evidence: "Commit 8027937: all five spokes deepened with facts from AKC standards, ACAAI, JACI 2012, Science 2009, Nature Communications 2016, Scientific Reports 2024, JVIM 2019, VCA, UC Davis VGL and AAHA, ledgered privately; hub cards on the breed guide and sibling links on every spoke"}
  ...
  - {id: g2-desc-long, closed: 2026-09-24, evidence: "Commit bb9b792: puppies 176 to 154, privacy 158 to 144; no description on the site over 155"}
  - {id: g2-heading-skip, closed: 2026-09-24, evidence: "Commit bb9b792: 13 pages promoted H3 to h2.h3 with matching CSS; audit finds no page skipping a level; computed styles unchanged"}
...
decisions:
  ...
  - {date: 2026-09-24, decision: "Alex: guide pages may use facts from reputable outside sources (breed standards and clubs, veterinary bodies, university labs, peer-reviewed studies, allergy associations, ASPCA), never other breeders or marketplaces. Health content is general and sourced only, with nothing about Troy, Finch or the litter beyond what the site already states. Sources are NOT shown on the page; a private ledger in source-files pairs each fact with its source. Pages publish first and Hope reviews after. Comparison page uses official parent-breed sizes, says Mini and Micro ranges vary by breeder, and never labels the puppies Mini"}
